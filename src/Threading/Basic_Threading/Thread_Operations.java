...
class YourextraTask extends CA implements Runnable {

    String document;
    int copies;

    YourextraTask(String document, int copies) {
        this.document = document;
        this.copies = copies;
    }
...
public class Thread_Operations {
    ...
    public static void main(String[] args) {
        ...
        Runnable y = new YourextraTask("--LearningC++.pdf--", 10);
        ...
        Thread zref = new Thread(y);

        mRef.setName("Java Thread");
        yRef.setName("Python Thread");
        zref.setName("C++ Thread");

        Thread.currentThread().setName("Ritesh");
        //--------------Pre-Defined Priority------------//
//        mRef.setPriority(Thread.MIN_PRIORITY);						// 0
//        yRef.setPriority(Thread.NORM_PRIORITY);						// 5
//        Thread.currentThread().setPriority(Thread.MAX_PRIORITY);	    // 10

        //Note: Priority Values cannot be Negative or Float it gives IllegalArgumentException and Priority values is only between 0 to 10 (int)values
        //--------------Manually Setting Priority------------//

        mRef.setPriority(4);                        // 4
        yRef.setPriority(10);                        // 11
        zref.setPriority(5);                        //8
        ...
        for (int i = 1; i <= 10; i++) {
            ...
            System.out.println(">> [Main] Prinitng **Ritesh's Profile.pdf** copy#" + i);
        }

        System.out.println(">> mRef Name: " + mRef.getName() + " and priority: " + mRef.getPriority() + " " + mRef.getId());
        System.out.println(">> yRef Name: " + yRef.getName() + " and priority: " + yRef.getPriority() + " " + yRef.getId());
        System.out.println(">> zRef Name: " + zref.getName() + " and priority: " + zref.getPriority() + " " + zref.getId());
        //Thread.currentThread() here shall give us the reference of main thread as we dont have it
        System.out.println(">> main Name: " + Thread.currentThread().getName() + " and priority: " + Thread.currentThread().getPriority() + " " + mRef.getId());
